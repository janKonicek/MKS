...
void sct_led(uint32_t value) {
	...
	for (uint32_t i = 0; i < 32; i++) {
		HAL_GPIO_WritePin(SCT_SDI_GPIO_Port, SCT_SDI_Pin, value & 1);	// Write value to SDI
		value >>= 1;
		HAL_GPIO_WritePin(SCT_CLK_GPIO_Port, SCT_CLK_Pin, 1);	// Pulse on CLK
		HAL_GPIO_WritePin(SCT_CLK_GPIO_Port, SCT_CLK_Pin, 0);
	}

	HAL_GPIO_WritePin(SCT_NLA_GPIO_Port, SCT_NLA_Pin, 1);	// Pulse on LA
	HAL_GPIO_WritePin(SCT_NLA_GPIO_Port, SCT_NLA_Pin, 0);
}

void sct_value(uint16_t value, uint8_t led, uint8_t DP) {
	uint32_t reg = 0;

	reg |= reg_values[3][led];
	if (DP) reg |= reg_values[SEG_NUM - 1 - DP][10];

	reg |= reg_values[0][value / 100 % 10];
	reg |= reg_values[1][value / 10 % 10];
	reg |= reg_values[2][value % 10];
	sct_led(reg);	// Show value on display
