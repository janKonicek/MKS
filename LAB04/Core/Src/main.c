...
#define SWITCH_DELAY 1000	// in ms
...
static volatile uint32_t raw_temp=1000;
static volatile uint32_t raw_volt=2000;
...
void button(void) {
	...
	if ((lastTime + SWITCH_DELAY) < HAL_GetTick()) {
		state = SHOW_POT;
	}
...
int main(void)
{
  ...
  /* USER CODE BEGIN WHILE */
    ...
    /* USER CODE BEGIN 3 */
		...
		switch (state) {
			case SHOW_POT:
				sct_value((raw_pot * 501) / 4096, (raw_pot * 9) / 4096);// Get and show value from encoder on display
				HAL_Delay(50);
				break;
